...
import { listDriverSignupsTable } from "@/lib/db/queries/admin.queries";
...
export default async function AdminDriversPage() {
  ...
  try {
    ...
    rows = await listDriverSignupsTable(admin, 200);
  } catch (e: unknown) {
  ...
  return (
    <div className="space-y-4">
      <h1 className="font-heading text-3xl tracking-tight">Driver signups</h1>
      <Card>
        <CardHeader>
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={String(r.id)}>
                  ...
                  <TableCell>{String(r.available_seats)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
